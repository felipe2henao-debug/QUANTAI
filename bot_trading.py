# ============================================================
# 🧠 BOT DE TRADING BTC - SISTEMA QUANT CON IA v2.2 (PROD)
# Autor: Pipe Henao
# Entorno: GitHub Actions (cron cada hora)
# Estrategia: Multi-timeframe 1H/4H | R/R >= 2.0 | IA Gemini JSON
# ============================================================
...
def calcular_rr(entrada, sl, tp, es_compra):
    if not all(isinstance(x, (int, float)) for x in [entrada, sl, tp]) or 0 in [entrada, sl, tp]:
        return 0.0
...
# ============================================================
# --- 6. CEREBRO IA — PROMPT MULTI-TIMEFRAME (JSON) ---
# ============================================================
...
def consultar_ia(df_1h, df_4h, macro):
    ...
    === REGLAS OBLIGATORIAS ===
    1. ALINEACION: Solo operar si 1H coincide con 4H. Contradiccion = ESPERA.
    2. R/R: Usa el ATR para calcular niveles. SL aprox 1.0x ATR, TP aprox 2.0x ATR (minimo).
       - Si el R/R proyectado es < 2.0, tu decision DEBE ser ESPERA.
    3. FORMATO: Debes devolver tu analisis ESTRICTAMENTE en el siguiente formato JSON.
    
    === FORMATO DE SALIDA (JSON PURO) ===
    Opciones para decision: "COMPRA", "VENTA" o "ESPERA".
    Opciones para alineacion_timeframes: "SI" o "NO".
    ...
    {{
      ...
      "tesis": "Tu justificacion tecnica breve (max 3 lineas)."
    }}
    """
    ...
    try:
        ...
        # Sanitización contra Markdown y tokens residuales de "Thinking"
        if "```" in texto:
        ...
        return {
            "decision": "ERROR", 
            "entrada": 0.0, "sl": 0.0, "tp": 0.0, "ratio_rr": 0.0,
            "alineacion_timeframes": "NO",
            "tesis": f"Fallo en la API de IA o parseo JSON: {str(e)}"
        }
...
def generar_grafico(df_1h, entrada=None, sl=None, tp=None):
    ...
    if all(v is not None and v > 0 for v in (entrada, sl, tp)):
...
def guardar_en_csv(df_1h, macro, analisis_dict, rr_calculado):
    try:
        decision = analisis_dict.get("decision", "ERROR").upper()
        ...
        registro = {
            ...
            "Tesis_IA"          : analisis_dict.get("tesis", "Sin tesis").replace("\n", " ").strip(),
        }
        ...
        print(f"✅ CSV guardado: {decision} | R/R: {rr_calculado}")
    except Exception as e:
        print(f"❌ Error CSV: {e}")
...
def ejecutar_analisis():
    ...
    # 3. Consulta a la IA (devuelve diccionario JSON)
    analisis = consultar_ia(df_1h, df_4h, macro)
    
    # 4. Leer niveles de forma segura (prevenir TypeError con null/None)
    decision = analisis.get("decision", "ESPERA").upper()
    entrada  = float(analisis.get("entrada") or 0.0)
    sl       = float(analisis.get("sl") or 0.0)
    tp       = float(analisis.get("tp") or 0.0)
    tesis    = analisis.get("tesis", "Sin tesis")
    
    print(f"\n🤖 IA Tesis:\n{tesis}\n")

    es_compra = decision == "COMPRA"
    es_venta  = decision == "VENTA"
    
    # Validación matemática propia (calcular solo si hay intención de operar)
    rr = 0.0
    ...
    # 6. Notificar por Telegram solo si la señal es válida y matemática correcta
    if senal_valida:
        ...
        print(f"⚠️  Señal de {decision} descartada — R/R matemático={rr} < 2.0")
    else:
        print("🔕 Sin oportunidad (ESPERA / ERROR)")
